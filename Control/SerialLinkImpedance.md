...
Given a desired trajectory defined by:
 ...
 the torque $\boldsymbol{\tau}\in\mathbb{R}^n$ to control the joint is computed as:
...
> [!WARNING]
> The `SerialLinkImpedance` class cannot gaurantee joint limit avoidance like the `SerialLinkKinematic` or `SerialLinkDynamic` classes.
...
We can define a wrench acting on the endpoint of the robot arm as:
```math
  \mathbf{w} =
  \begin{bmatrix}
    \mathbf{f} \\
...
\end{align}
```
so the joint torques that the wrench induces on the endpoint are:
```math
...
```
so it is orthogonal to the Jacobian, and
```math
...
I do not invert the Jacobian in this class, so there's no problem 😉
