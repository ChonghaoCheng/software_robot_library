/**
 * @file TrajectoryTracking/detail/RmpccResidualLinearization.h
    ...
    ReferenceTangentFunction &&referenceTangent)
{
    ...
    const RmpccErrorProjection projection = rmpcc_error_projection(
        errorTangent, metric, geometry, regularization);
    ...
    ReferenceTangentFunction &&referenceTangent)
{
    ...
    result.residual = rmpcc_projected_residuals(
        state, metric, geometry, regularization, tangent);
    for(int column = 0; column < 7; ++column)
    {
        RmpccStateVector plus = state;
        RmpccStateVector minus = state;
        ...
        const RmpccFullScrewResiduals plusResidual =
            rmpcc_projected_residuals(
                plus, metric, geometry, regularization, tangent);
        const RmpccFullScrewResiduals minusResidual =
            rmpcc_projected_residuals(
                minus, metric, geometry, regularization, tangent);
        result.contourJacobian.col(column) =
...
rmpcc_full_screw_residuals(
    ...
    ReferenceTangentFunction &&referenceTangent)
{
    return rmpcc_projected_residuals(
        state, metric, RmpccLagGeometry::FullScrew, regularization,
        referenceTangent);
}
...
rmpcc_linearize_full_screw_residuals(
    ...
    ReferenceTangentFunction &&referenceTangent)
{
    return rmpcc_linearize_projected_residuals(
        state, metric, RmpccLagGeometry::FullScrew, regularization,
        finiteDifferenceStep, referenceTangent);
}
