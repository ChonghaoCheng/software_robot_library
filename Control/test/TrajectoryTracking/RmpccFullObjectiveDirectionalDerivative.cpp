...
{
    const auto residual = RobotLibrary::Control::rmpcc_projected_residuals(
        state, metric, geometry, 1e-10, tangent);
...
int main()
{
    ...
    constexpr double boundaryObjectiveStep = 5e-8;
                                 ...
                                 1.0 - 0.5 * residualStep, 0.4})
        {
            ...
            const auto linearization =
                RobotLibrary::Control::rmpcc_linearize_projected_residuals(
                    geometryState, metric, geometry, 1e-10,
                    residualStep, tangent);
            const State gradient =
                2.0 * linearization.contourJacobian.transpose()
                      * contourWeight * linearization.residual.contour
                + 2.0 * linearization.lagJacobian.transpose()
                      * lagWeight * linearization.residual.lag;
            double expectedDerivative = 0.0;
            if(progress == 0.0 or progress == 1.0)
            {
                expectedDerivative =
                    (objective(geometryState
                                   + boundaryObjectiveStep * feasibleDirection,
                               metric, contourWeight, lagWeight, geometry)
                     - objective(geometryState, metric, contourWeight,
                                 lagWeight, geometry))
                    / boundaryObjectiveStep;
            }
            else
            ...
            const double error = relative_error(
                gradient.dot(feasibleDirection), expectedDerivative);
            if(error > 1e-5)
            {
                std::cerr << "Projected objective derivative mismatch for geometry "
